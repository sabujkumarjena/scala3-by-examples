...
object OrganizingCAs {
  ...
  object EmployeGivens {
    ...
    extension (e: Employee)
      def greet: String = s"Hello, I am ${e.name}"
  }
...
//  import EmployeGivens. {given Ordering[Employee]}
  ...
  object Purchase {
    given purchaseOrdering: Ordering[Purchase] with
      override def compare(x: Purchase, y: Purchase): Int = (y.nUnits * y.unitPrice - x.nUnits * x.unitPrice).asInstanceOf[Int]
  }
  ...
  object UnitPriceOrdering {
    given  Ordering[Purchase] = Ordering.fromLessThan((x,y) => x.unitPrice < y.unitPrice)
  }

  val purchases: List[Purchase] = List(Purchase(3, 12.4), Purchase(5, 1), Purchase(2, 50.5))
